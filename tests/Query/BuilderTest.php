...
class BuilderTest extends TestCase
{
    ...
    public function testKeyCastsDoNotConvertPatternOperators(): void
    {
        $convert = static fn (mixed $value): mixed => $value === 'foo' ? 'converted' : $value;

        // "like" and "regex" values are string patterns, they must not be converted
        $mql = $this->getBuilder()
            ->setKeyCasts(['author_id' => $convert])
            ->where('author_id', 'like', 'foo%')
            ->where('author_id', 'regex', '/foo/')
            ->toMql();

        [$like, $regex] = $mql['find'][0]['$and'];

        $this->assertSame('^foo.*$', $like['author_id']->getPattern());
        $this->assertSame('foo', $regex['author_id']->getPattern());
    }
